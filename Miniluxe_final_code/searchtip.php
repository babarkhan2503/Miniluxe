...
// b.engage_id,b.start_time, d.technician_id, d.technician_name,b.status, b.rating,b.total_cost,b.tip,b.service_id
$aColumns = array("b.engage_id", "b.start_time","d.technician_id", "d.technician_name", "b.status", "b.rating","b.total_cost","b.tip","b.service_id","b.user_id");
...
LIKE '%" . mysql_real_escape_string($_GET['sSearch']) . "%' OR ";
    ...
    $sWhere .= ") && (b.technician_id = d.technician_id) && (b.status =6) && (b.start_time between '".$from."' AND  '".$to."')";
}
...
LIKE '%" . mysql_real_escape_string($_GET['sSearch']) . "%' OR ";
    ...
    $sWhere = substr_replace($sWhere, "", -3);
    $sWhere .= ") && (b.technician_id = d.technician_id) && (b.status =6)";
}
    ...
    $sWhere .= " WHERE (b.technician_id = d.technician_id) && (b.status =6) && (b.start_time between '".$from."' AND  '".$to."')";
}
else
{
    $sWhere = "WHERE (b.technician_id = d.technician_id) && (b.status =6)";
}
...
$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS b.engage_id,b.start_time, d.technician_id, d.technician_name,b.status, b.rating,b.total_cost,b.tip,b.service_id,b.user_id FROM tb_engagements b, tb_technicians d
		$sWhere
		$sOrder
		$sLimit
		";
...
$sQuery = "
		...
		FROM tb_engagements b, tb_technicians d 
                $sWhere
	";
...
$sqla = "SELECT  b.engage_id,a.service_name FROM tb_services a, tb_engagements b, tb_user_services f WHERE (b.status = 6) && (f.service_id = a.id) && (b.engage_id = f.engage_id)";
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    $row[9] = '';
        ...
        if ( $row[0] == $response15[$k]['engage_id']) {
            ...
            $row[9] .= $response15[$k]['service_name'] . ",";
